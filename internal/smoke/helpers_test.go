...
// newJar returns the cookie jar every harness HTTP client uses. The WebAuthn
// ceremony carries its sealed challenge between begin and finish in a Secure
// cookie, and since Go 1.26 net/http/cookiejar treats a plain-HTTP localhost
// origin as a secure context, matching browser behaviour, so a plain jar
// sends it (the issue #78 wrapper this replaced is gone with the toolchain
// bump).
